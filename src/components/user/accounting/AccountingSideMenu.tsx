...
import style from "./styles/accountingSideMenu.module.scss";
...
type AccountingSideMenuProps = {
  activeKey: AccountingMenuKey;
};

const accountingLinks: Array<{
  key: AccountingMenuKey;
  label: string;
  href: string;
  icon?: JSX.Element;
}> = [
  ...
  { key: "wallet", icon: <LuWallet />, label: "Wallet", href: "/user/wallet" },
  {
    key: "virtual-wallet",
    ...
    href: "/user/virtual-wallet",
  },
  {
    key: "participate-and-win",
    label: "Referral Program",
    icon: <GoPeople />,
    href: "/user/participate-and-win",
  },
...
export const AccountingSideMenu = ({ activeKey }: AccountingSideMenuProps) => {
  const { logout, isLoggingOut } = useLogout();

  return (
    <aside className={style.sideCard}>
      {accountingLinks.map((item) => (
        <Link
          key={item.key}
          href={item.href}
          className={`${style.sideItem} ${item.key === activeKey ? style.activeItem : ""}`}
        >
      ...
      <button
        ...
        className={`${style.sideItem} ${style.logoutItem}`}
      >
        <FiLogOut />
